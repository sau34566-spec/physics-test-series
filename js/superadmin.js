...
async function api(action, payload = {}) {
  const read = ["overview","listInstitutes","listAdmins","getInstitute","history","audit","catalog","settings","validate","provisioning"].includes(action);
...
async function navigate(name) {
  ...
  try {
    if(name==="Dashboard") {
      ...
      const cards=[["Total Institutes",d.counts.totalInstitutes],...["ACTIVE","SUSPENDED","PENDING","INACTIVE","ARCHIVED"].map(k=>[k,d.counts[k]]),["Active Institute Admins",d.counts.activeAdmins],["Disabled / Suspended Admins",d.counts.disabledAdmins],["Failed Administrative Operations",d.counts.failedOperations]];
      ...
      $("content").innerHTML='<div class="panel"><h2>Platform defaults</h2><p>These settings do not rewrite existing institute configurations.</p><div class="grid">'+field("Platform name","platformName",settings.platformName)+check("Maintenance mode: block new portal entry","maintenanceMode",settings.maintenanceMode)+select("Default institute type","defaultType",settings.defaults?.instituteType||"SCHOOL",catalog.types)+select("Default exam mode","defaultMode",settings.defaults?.examMode||"NORMAL",["NORMAL","CBT"])+select("Default login template","defaultLogin",settings.defaults?.loginTemplate||"login-clean",catalog.templates.filter(t=>t.templateType==="login"&&t.status==="ACTIVE"&&t.version===1).map(t=>t.templateId))+select("Default instruction template","defaultInstruction",settings.defaults?.instructionTemplate||"instructions-clean",catalog.templates.filter(t=>t.templateType==="instructions"&&t.status==="ACTIVE"&&t.version===1).map(t=>t.templateId))+'</div><div class="actions"><button data-action="saveSettings">Save platform settings</button></div><p class="muted">Version '+settings.version+'</p></div>';
...
$("content").addEventListener("click",e=>{
  ...
  run(async()=>{
    ...
    if(b.dataset.resumeProvision!==undefined){
      const op=listState.items[Number(b.dataset.resumeProvision)];
      if(op.actorUid!==auth.currentUser.uid)throw new Error("The administrator who started this request must resume it.");
      await endpoint({action:"createAdmin",payload:{name:op.name,email:op.email,phone:op.phone,instituteId:op.instituteId},requestId:op.requestId});
      notice("Administrator provisioning completed.");await navigate("Institute Admins");return;
    }
    ...
    if(action==="newPreset"){
      $("content").innerHTML='<div class="panel"><h2>Create Security Preset Version</h2><div class="grid"><label>Preset ID<input id="presetId" value="custom"></label><label>Name<input id="presetName" value="Custom Security"></label>'+S.SECURITY.map(k=>'<label class="check"><input type="checkbox" data-preset-security="'+k+'">'+esc(k)+'</label>').join("")+'</div><button data-action="savePreset">Publish New Version</button></div>';
    }
    if(action==="savePreset"){
      const next=clone(catalog),id=$("presetId").value.trim(),name=$("presetName").value.trim();
      const version=Math.max(0,...next.presets.filter(p=>p.id===id).map(p=>p.version))+1;
      const values=Object.fromEntries([...document.querySelectorAll("[data-preset-security]")].map(e=>[e.dataset.presetSecurity,e.checked]));
      if(values.focusLock&&!values.fullscreen)throw new Error("Focus Lock requires fullscreen.");
      next.presets.push({id,name,version,settings:values});await saveRegistry(next);
    }
    if(action==="provisioning"){
      const response=await api("provisioning");listState.items=response.items;
      $("content").innerHTML='<div class="panel"><h2>Provisioning Recovery</h2><p>Retry uses the original request ID. Existing-email conflicts need Authentication reconciliation; a new email is required if it belongs to another account.</p>'+response.items.map((o,i)=>'<div class="item"><strong>'+esc(o.email)+'</strong><p>'+esc(o.status)+' · '+esc(o.errorCode||"")+'</p><button data-resume-provision="'+i+'">Retry Provisioning</button></div>').join("")+(!response.items.length?'<p>No pending provisioning operations.</p>':"")+'</div>';
    }
    ...
    if(action==="saveSettings"){const name=document.querySelector('[data-path="platformName"]').value,maintenance=document.querySelector('[data-path="maintenanceMode"]').checked;const mode=document.querySelector('[data-path="defaultMode"]').value;
      const defaults={instituteType:document.querySelector('[data-path="defaultType"]').value,examMode:mode,examTemplate:mode==="CBT"?"exam-cbt":"exam-normal",
        loginTemplate:document.querySelector('[data-path="defaultLogin"]').value,instructionTemplate:document.querySelector('[data-path="defaultInstruction"]').value,
        features:{...S.defaults().features,cbt:mode==="CBT",palette:mode==="CBT"}};
      if(!confirm("Save platform settings?"))return;await api("saveSettings",{platformName:name,maintenanceMode:maintenance,defaults,expectedVersion:settings.version});await navigate("Platform Settings");notice("Platform settings saved.");}
