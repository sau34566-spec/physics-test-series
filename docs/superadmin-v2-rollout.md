...
5. Admin provisioning retry uses the same request ID. Partial Auth/profile failures preserve the reservation and do not grant admin access, and the initiating Super Admin can retry from Provisioning Recovery. Administrative reconciliation/cancellation is still required for existing-email collisions and abandoned reservations.
...
7. Security presets support immutable new versions; platform default type/mode/templates are configurable. Failed-operation counts and last-admin-activity are recorded. Configuration checks are not a substitute for detailed operational health probes.
